...
M3Result m3_InjectFunction(IM3Module i_module,
  ...
  if (index >= 0) {
    ...
    if (not AreFuncTypesEqual(ftype, function->funcType)) _throw ("function type mismatch");
  } else {
  ...
  if (i_doCompilation and not i_module->runtime) _throw ("module must be loaded into runtime to compile function");
