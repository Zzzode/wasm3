...
M3Result EnsureCodePageNumLines(IM3Compilation o, u32 i_numLines) {
  ...
  if (NumFreeLines(o->page) < i_numLines) {
    ...
    if (page) {
      m3log (emit,
             "bridging new code page from: %d %p (free slots: %d) to: %d",
             o->page->info.sequence,
             GetPC(o),
             NumFreeLines(o->page),
             page->info.sequence);
...
static const IM3Operation
    c_intSelectOps[2][4] = {{op_Select_i32_rss, op_Select_i32_srs, op_Select_i32_ssr, op_Select_i32_sss},
                            {op_Select_i64_rss, op_Select_i64_srs, op_Select_i64_ssr, op_Select_i64_sss}};
#if d_m3HasFloat
static const IM3Operation
    c_fpSelectOps[2][2][3] = {{{op_Select_f32_sss, op_Select_f32_srs, op_Select_f32_ssr},        // selector in slot
...
M3Result AllocateConstantSlots(IM3Compilation o, u16 *o_slot, u8 i_type) {
  ...
  return AllocateSlotsWithinRange(o,
                                  o_slot,
                                  i_type,
                                  o->slotFirstConstIndex,
                                  M3_MIN(o->slotFirstDynamicIndex, maxTableIndex));
}
  ...
  *o_preservedSlotNumber = (u16) i_localSlot;
  ...
  for (u32 i = startIndex; i < o->stackIndex; ++i) {
    if (o->wasmStack[i] == i_localSlot) {
      ...
      } else _               (IncrementSlotUsageCount(o, *o_preservedSlotNumber));
...
static
M3Result CopyStackSlotsR(IM3Compilation o,
                         u16 i_targetSlotStackIndex,
                         u16 i_stackIndex,
                         u16 i_endStackIndex,
                         u16 i_tempSlot) {
  M3Result result = m3Err_none;
...
M3Result Compile_SetLocal(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (localIndex < GetFunctionNumArgsAndLocals(o->function)) {
    ...
    u16 preserveSlot;
    _       (FindReferencedLocalWithinCurrentBlock(o,
                                                   &preserveSlot,
                                                   localSlot));  // preserve will be different than local, if referenced

    if (preserveSlot == localSlot) _           (CopyStackTopToSlot(o, localSlot))
    else _           (PreservedCopyTopSlot(o, localSlot, preserveSlot))
...
M3Result Compile_GetSetGlobal(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (globalIndex < o->module->numGlobals) {
    ...
    } else _throw (ErrorCompile(m3Err_globalMemoryNotAllocated,
                                o,
                                "module '%s' is missing global memory",
                                o->module->name));
...
#   endif
  ...
  } else if (not IsStackPolymorphic(o)) _throw (m3Err_functionStackUnderrun);
...
M3Result Compile_Convert(IM3Compilation o, m3opcode_t i_opcode) {
  _try
  {
    ...
    if (destInSlot) _       (PushAllocatedSlotAndEmit(o, opInfo->type))
    else _       (PushRegister(o, opInfo->type))
...
M3Result CompileRawFunction(IM3Module io_module,
                            IM3Function io_function,
...
# endif
        ...
        d_m3DebugOp (GetGlobal_s32), d_m3DebugOp (GetGlobal_s64), d_m3DebugOp (ContinueLoop), d_m3DebugOp (
            ContinueLoopIf),

        d_m3DebugOp (CopySlot_32), d_m3DebugOp (PreserveCopySlot_32), d_m3DebugOp (If_s), d_m3DebugOp (
            BranchIfPrologue_s),
        d_m3DebugOp (CopySlot_64), d_m3DebugOp (PreserveCopySlot_64), d_m3DebugOp (If_r), d_m3DebugOp (
            BranchIfPrologue_r),

        d_m3DebugOp (Select_i32_rss), d_m3DebugOp (Select_i32_srs), d_m3DebugOp (Select_i32_ssr), d_m3DebugOp (
            Select_i32_sss),
        d_m3DebugOp (Select_i64_rss), d_m3DebugOp (Select_i64_srs), d_m3DebugOp (Select_i64_ssr), d_m3DebugOp (
            Select_i64_sss),
...
IM3OpInfo GetOpInfo(m3opcode_t opcode) {
  switch (opcode >> 8) {
    case 0x00:
      ...
      break;
    case c_waOp_extended:opcode &= 0xFF;
...
# endif
    ...
    if (not o->function) {
      switch (opcode) {
        ...
        case c_waOp_end:break;
        default: _throw(m3Err_restrictedOpcode);
      }
    }
...
M3Result PushBlockResults(IM3Compilation o) {
  ...
  for (u16 i = 0; i < numResults; ++i) {
    ...
    if (i == numResults - 1 and IsFpType(type)) {
      _           (PushRegister(o, type));
    } else _           (PushAllocatedSlot(o, type));
  }
...
M3Result CompileLocals(IM3Compilation o) {
  ...
  for (u32 l = 0; l < numLocalBlocks; ++l) {
    ...
    while (varCount--) _           (PushAllocatedSlot(o, localType));
  }
...
M3Result CompileFunction(IM3Function io_function) {
  ...
  m3log (compile,
         "compiling: [%d] %s %s; wasm-size: %d",
         io_function->index,
         m3_GetFunctionName(io_function),
         SPrintFuncTypeSignature(funcType),
         (u32) (io_function->wasmEnd - io_function->wasm));
  IM3Runtime runtime = io_function->module->runtime;

  IM3Compilation o = &runtime->compilation;
  d_m3Assert (d_m3MaxFunctionSlots
                  >= d_m3MaxFunctionStackHeight * (d_m3Use32BitSlots + 1))  // need twice as many slots in 32-bit mode
  ...
  _try
  {
    ...
    o->slotMaxAllocatedIndexPlusOne = o->function->numRetAndArgSlots = o->slotFirstLocalIndex =
        o->slotFirstDynamicIndex;
    ...
    m3log (compile,
           "unique constant slots: %d; unused slots: %d",
           numConstantSlots,
