...
namespace wasm3{
    ...
    namespace detail {
      ...
      template < typename
      T, typename...> struct first_type { typedef T type; };

      typedef const void *(*m3_api_raw_fn)(IM3Runtime, uint64_t *, void *);

      template < typename
      T >
      void arg_from_stack(T
      &dest, stack_type & _sp, mem_type
      mem) {
        m3ApiGetArg(T, tmp);
        dest = tmp;
      }
      ...
      static void get_args_from_stack(stack_type
      &sp, mem_type
      mem, std::tuple < Args...> &tuple) {
        std::apply([&](auto &... item) {
    ...
    class runtime {
      ...
      runtime(
      const std::shared_ptr<M3Environment>
      &env, size_t
      stack_size_bytes)
      : m_env(env)
      {
    ...
    class module {
      ...
      void parse(IM3Environment env, const uint8_t *data, size_t size) {
        ...
        m_module.reset(p,[this](IM3Module
        module) {
          if (!m_loaded) {
    ...
    class function {
      ...
      Ret, typename ... Args >
          typename
      detail::first_type < Ret,
          typename
      std::enable_if<std::is_convertible<Args, const char *>::value>::type...>::type
      call_argv(Args... args) {
      ...
      function(
      const std::shared_ptr<M3Runtime>
      &runtime,
      const char *name) : m_runtime(runtime)
      {
